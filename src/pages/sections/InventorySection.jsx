...
const InventorySection = () => {
  ...
  const updateQty = (id, delta) => {
    setIngredients(prev => prev.map(i => i.ingredientId === id ? {
      ...i,
  ...
  return <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          color: '#8B6E52'
        }}>{lowStock.length} items need restocking</p>
        </div>
      ...
      {/* Low stock alert */}
      {lowStock.length > 0 && <div className="rounded-2xl p-4" style={{
      background: '#FEF2F2',
      border: '1px solid #FECACA'
          ...
          <div className="flex flex-wrap gap-2">
            {lowStock.map(ing => <span key={ing.ingredientId} className="px-3 py-1 rounded-full text-xs font-semibold" style={{
          background: '#FEE2E2',
          color: '#DC2626'
        }}>
      ...
      border: '1px solid #E8D5C0'
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search ingredients..." className="bg-transparent text-sm outline-none flex-1" style={{
        color: '#2C1810'
      }} />
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            ...
            background: '#F5E6D3'
              ...
              {['Ingredient', 'Stock', 'Unit', 'Min. Threshold', 'Status', canEdit ? 'Actions' : ''].filter(Boolean).map(h => <th key={h} className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider" style={{
              color: '#6B4F3A'
            }}>{h}</th>)}
            </tr>
          </thead>
          <tbody>
            ...
            return <tr key={ing.ingredientId} className="border-t transition-colors hover:bg-amber-50/30" style={{
              borderColor: '#F0E8DE'
            }}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      {isLow ? <AlertTriangle size={14} className="text-red-500" /> : <CheckCircle size={14} className="text-green-500" />}
                  ...
                  <td className="px-4 py-3">
                    ...
                    background: '#F0E8DE'
                      ...
                      {canEdit && <div className="flex items-center gap-1">
                          <button onClick={() => updateQty(ing.ingredientId, -1)} className="w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center" style={{
                      background: '#FEE2E2',
                      color: '#DC2626'
                    }}>−</button>
                          <button onClick={() => updateQty(ing.ingredientId, 1)} className="w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center" style={{
                ...
                color: '#8B6E52'
                  ...
                  {canEdit && <td className="px-4 py-3">
                      <button onClick={() => openEdit(ing)} className="text-xs px-3 py-1 rounded-lg font-medium" style={{
                  background: '#F0E8DE',
                  color: '#8B3A0F'
                }}>Edit</button>
                    </td>}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                color: '#6B4F3A'
                  ...
                  <input type="number" value={form.quantity} onChange={e => setForm(f => ({
                ...f,
              ...
              <div>
                <label className="text-xs font-semibold uppercase tracking-wider block mb-1" style={{
              color: '#6B4F3A'
            }}>Minimum Threshold</label>
                <input type="number" value={form.threshold} onChange={e => setForm(f => ({
              ...f,
