...
const statusColors: Record<string, { bg: string; color: string }> = {
  pending: { bg: '#FEF9EE', color: '#C8862A' },
  confirmed: { bg: '#EFF6FF', color: '#0369A1' },
  preparing: { bg: '#FFFBEB', color: '#D97706' },
  ready: { bg: '#F0FDF4', color: '#059669' },
  served: { bg: '#F3F4F6', color: '#6B7280' },
...
const OrdersSection: React.FC = () => {
  const { orders, setOrders, currentUser, tables } = useApp();
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [showNewOrder, setShowNewOrder] = useState(false);
  const [selectedOrder, setSelectedOrder] = useState<Order | null>(null);
  const [newOrderForm, setNewOrderForm] = useState({ customerName: '', tableId: '', type: 'dine-in' as Order['type'], notes: '' });
  const [orderItems, setOrderItems] = useState<{ itemId: number; qty: number }[]>([]);
  ...
  const updateStatus = (orderId: number, newStatus: string) => {
    ...
    if (selectedOrder?.orderId === orderId) setSelectedOrder(prev => prev ? { ...prev, status: newStatus as Order['status'] } : null);
  };
  ...
  const createOrder = () => {
    ...
    const items: OrderItem[] = orderItems.map((oi, idx) => {
      const menu = allMenuItems.find(m => m.itemId === oi.itemId)!;
      return { orderItemId: Date.now() + idx, itemId: oi.itemId, itemName: menu.name, quantity: oi.qty, unitPrice: menu.price, subTotal: menu.price * oi.qty };
    });
    ...
    setNewOrderForm({ customerName: '', tableId: '', type: 'dine-in', notes: '' });
    setOrderItems([]);
  };
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black" style={{ color: '#2C1810', fontFamily: "'Playfair Display', serif" }}>Orders</h2>
          <p className="text-sm" style={{ color: '#8B6E52' }}>{orders.filter(o => !['paid', 'cancelled'].includes(o.status)).length} active orders</p>
        </div>
        {canCreate && (
        ...
        <div className="flex gap-2 flex-wrap">
          {['all', 'pending', 'confirmed', 'preparing', 'ready', 'served', 'paid', 'cancelled'].map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              ...
              style={statusFilter === s ? { background: '#C8862A', color: 'white' } : { background: 'white', color: '#8B6E52', border: '1px solid #E8D5C0' }}>
              {s}
            </button>
          ))}
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        {filtered.map(order => {
          ...
          return (
            <div key={order.orderId} className="rounded-2xl p-5 shadow-sm hover:shadow-md transition-all cursor-pointer"
              style={{ background: 'white', border: '1px solid #F0E8DE' }}
              onClick={() => setSelectedOrder(order)}>
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-black text-base" style={{ color: '#2C1810', fontFamily: "'Playfair Display', serif" }}>#{order.orderId}</span>
                    <span className="px-2 py-0.5 rounded-full text-xs font-semibold capitalize" style={sc}>{order.status}</span>
                    <span className="px-2 py-0.5 rounded-full text-xs font-medium capitalize" style={{ background: '#F5E6D3', color: '#8B6E52' }}>{order.type}</span>
                  </div>
              ...
              {canUpdateStatus && nextStatus && (
                <button onClick={e => { e.stopPropagation(); updateStatus(order.orderId, nextStatus); }}
                  className="w-full py-2 rounded-xl text-sm font-medium transition-all hover:opacity-90"
                  style={{ background: '#F0E8DE', color: '#8B3A0F' }}>
                  → Mark as {nextStatus}
                </button>
              )}
              {order.status === 'served' && currentUser?.role === 'cashier' && (
                <button onClick={e => { e.stopPropagation(); updateStatus(order.orderId, 'paid'); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)' }}>
          <div className="w-full max-w-lg rounded-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto" style={{ background: 'white' }}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setSelectedOrder(null)} style={{ color: '#8B6E52' }}><X size={20} /></button>
            </div>
            ...
            <div className="border-t pt-4 mb-5" style={{ borderColor: '#F0E8DE' }}>
            ...
            <button onClick={() => setSelectedOrder(null)}
              className="w-full py-3 rounded-xl text-sm font-medium" style={{ background: '#F0E8DE', color: '#6B4F3A' }}>
              Close
            </button>
          </div>
        </div>
      ...
      {showNewOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)' }}>
          <div className="w-full max-w-2xl rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto" style={{ background: 'white' }}>
            <div className="p-6 border-b" style={{ borderColor: '#F0E8DE' }}>
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-bold" style={{ color: '#2C1810', fontFamily: "'Playfair Display', serif" }}>New Order</h3>
                <button onClick={() => setShowNewOrder(false)} style={{ color: '#8B6E52' }}><X size={20} /></button>
              </div>
            </div>
            <div className="p-6 grid md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                {[
                  { label: 'Customer Name', key: 'customerName', type: 'text', placeholder: 'Customer name' },
                ].map(field => (
                  <div key={field.key}>
                    <label className="text-xs font-semibold uppercase tracking-wider block mb-1" style={{ color: '#6B4F3A' }}>{field.label}</label>
                    <input type={field.type} placeholder={field.placeholder}
                      value={(newOrderForm as any)[field.key]}
                      onChange={e => setNewOrderForm(f => ({ ...f, [field.key]: e.target.value }))}
                ...
                <div>
                  ...
                  <select value={newOrderForm.type} onChange={e => setNewOrderForm(f => ({ ...f, type: e.target.value as Order['type'] }))}
                ...
                {newOrderForm.type === 'dine-in' && (
                  <div>
                    <label className="text-xs font-semibold uppercase tracking-wider block mb-1" style={{ color: '#6B4F3A' }}>Table</label>
                    <select value={newOrderForm.tableId} onChange={e => setNewOrderForm(f => ({ ...f, tableId: e.target.value }))}
                      className="w-full px-4 py-3 rounded-xl text-sm outline-none" style={{ border: '2px solid #E8D5C0', color: '#2C1810', background: 'white' }}>
                ...
                <div>
                  ...
                  <textarea value={newOrderForm.notes} onChange={e => setNewOrderForm(f => ({ ...f, notes: e.target.value }))} rows={2}
                    className="w-full px-4 py-3 rounded-xl text-sm outline-none resize-none" style={{ border: '2px solid #E8D5C0', color: '#2C1810' }} />
                </div>
                {/* Selected items */}
                <div>
                  <h4 className="font-semibold mb-2" style={{ color: '#2C1810' }}>Selected Items</h4>
                  {orderItems.length === 0 ? <p className="text-xs" style={{ color: '#8B6E52' }}>No items selected</p> : (
                          ...
                          <div key={oi.itemId} className="flex items-center justify-between text-sm p-2 rounded-lg" style={{ background: '#FDF6EE' }}>
                            ...
                            <div className="flex items-center gap-2">
                              <button onClick={() => setOrderItems(p => p.map(i => i.itemId === oi.itemId ? { ...i, qty: Math.max(0, i.qty - 1) } : i).filter(i => i.qty > 0))}
                                className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: '#F0E8DE', color: '#8B6E52' }}>−</button>
                              <span className="font-bold" style={{ color: '#C8862A' }}>{oi.qty}</span>
                              <button onClick={() => addItemToOrder(oi.itemId)}
                                className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: '#C8862A', color: 'white' }}>+</button>
                              <span className="text-xs ml-2" style={{ color: '#C8862A' }}>ETB {menu.price * oi.qty}</span>
                            </div>
                          </div>
                        );
                      })}
                      <div className="font-bold text-right" style={{ color: '#C8862A' }}>
                        Total: ETB {orderItems.reduce((s, oi) => { const m = allMenuItems.find(m => m.itemId === oi.itemId)!; return s + m.price * oi.qty; }, 0)}
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div>
                ...
                <div className="space-y-2 max-h-80 overflow-y-auto">
                  {allMenuItems.filter(m => m.availability).map(item => (
            ...
            <div className="p-6 border-t flex gap-3" style={{ borderColor: '#F0E8DE' }}>
              <button onClick={() => setShowNewOrder(false)} className="flex-1 py-3 rounded-xl text-sm font-medium"
                style={{ background: '#F0E8DE', color: '#6B4F3A' }}>Cancel</button>
              <button onClick={createOrder} className="flex-1 py-3 rounded-xl text-sm font-semibold"
                style={{ background: 'linear-gradient(135deg, #C8862A, #8B3A0F)', color: 'white' }}>
                Create Order
              </button>
            </div>
