...
import {
  ...
  Table2, Calendar, Package, CreditCard, BarChart3,
...
const navItems: { section: string; label: string; icon: React.ReactNode; roles: UserRole[] }[] = [
  { section: 'dashboard', label: 'Dashboard', icon: <LayoutDashboard size={18} />, roles: ['admin', 'manager', 'waiter', 'chef', 'cashier', 'customer'] },
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 h-full z-40 flex flex-col transition-all duration-300
        ...
        style={{ background: '#1A1008', boxShadow: '4px 0 24px rgba(0,0,0,0.3)' }}>
          ...
          {sidebarOpen && (
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs" style={{ color: '#C8862A' }}>RESTAURANT</p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-1 px-2">
            {visibleNav.map(item => (
              <button key={item.section}
                onClick={() => { onNavigate(item.section); if (window.innerWidth < 1024) setSidebarOpen(false); }}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all
                  ${activeSection === item.section ? 'text-white' : 'hover:opacity-100 opacity-60 hover:bg-white/5 text-white'}`}
                style={activeSection === item.section ? { background: `linear-gradient(135deg, ${roleColor}40, ${roleColor}20)`, borderLeft: `3px solid ${roleColor}` } : {}}>
                <span style={{ color: activeSection === item.section ? roleColor : 'inherit' }}>{item.icon}</span>
        ...
        <div className="px-2 py-4 border-t" style={{ borderColor: 'rgba(200,134,42,0.15)' }}>
          <button onClick={logout}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-red-400 hover:bg-red-500/10 transition-all opacity-70 hover:opacity-100">
            <LogOut size={18} />
