...
const PaymentsSection = () => {
  ...
  const processPayment = () => {
    ...
    setOrders(prev => prev.map(o => o.orderId === selectedOrderId ? {
      ...o,
      status: 'paid'
    } : o));
  ...
  return <div className="p-6 space-y-5">
      <div>
        ...
        color: '#8B6E52'
      }}>{unpaidOrders.length} orders awaiting payment</p>
      </div>

      {/* Revenue stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="rounded-2xl p-5 col-span-2" style={{
        ...
        border: '1px solid rgba(200,134,42,0.2)'
          ...
          color: '#6B4F3A'
        }}>From {payments.length} transactions</p>
        </div>
        {[{
        label: 'Cash',
        value: methodBreakdown.cash,
        icon: <Banknote size={18} />,
        color: '#059669',
        bg: '#ECFDF5'
      }, {
        label: 'Card',
        value: methodBreakdown.card,
        icon: <CreditCard size={18} />,
        color: '#0369A1',
        bg: '#EFF6FF'
      }, {
        label: 'Mobile',
        value: methodBreakdown.mobile,
        icon: <Smartphone size={18} />,
        color: '#7C3AED',
        bg: '#F5F3FF'
      }].map((m, i) => <div key={i} className="rounded-2xl p-4" style={{
        background: m.bg,
        border: `1px solid ${m.color}20`
      }}>
            <div className="flex items-center gap-2 mb-2" style={{
          color: m.color
        }}>{m.icon}<span className="text-xs font-semibold">{m.label}</span></div>
      ...
      {canProcess && unpaidOrders.length > 0 && <div>
        ...
        fontFamily: "'Playfair Display', serif"
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {unpaidOrders.map(order => <div key={order.orderId} className="rounded-2xl p-5 shadow-sm" style={{
          background: 'white',
          border: '2px solid #F0E8DE'
              ...
              fontFamily: "'Playfair Display', serif"
                ...
                <div className="space-y-1 mb-4">
                  {order.items.map(item => <div key={item.orderItemId} className="flex justify-between text-xs" style={{
              color: '#8B6E52'
            }}>
          ...
          }} className="w-full py-2.5 rounded-xl text-sm font-semibold transition-all hover:scale-105" style={{
          ...
          <table className="w-full text-sm">
            <thead>
              ...
              background: '#F5E6D3'
                ...
                {['Txn ID', 'Order', 'Amount', 'Method', 'Status', 'Date', 'Receipt'].map(h => <th key={h} className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider" style={{
                color: '#6B4F3A'
              }}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              ...
              return <tr key={p.paymentId} className="border-t hover:bg-amber-50/30" style={{
                borderColor: '#F0E8DE'
              }}>
                  ...
                  color: '#C8862A'
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 capitalize" style={{
                    color: '#6B4F3A'
                  }}>
                  ...
                  color: '#8B6E52'
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => setReceiptOrder(p.orderId)} className="flex items-center gap-1 text-xs px-2 py-1 rounded-lg" style={{
                    background: '#F5E6D3',
                    color: '#8B3A0F'
                  }}>
      ...
      {/* Payment Modal */}
      {showPayModal && <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{
        ...
        background: 'white'
          ...
          return order ? <div>
              ...
              border: '1px solid #E8D5C0'
                    ...
                    <p className="font-bold" style={{
                color: '#2C1810'
              }}>Order #{order.orderId} – {order.customerName}</p>
                    <p className="text-3xl font-black mt-2" style={{
                color: '#C8862A',
                fontFamily: "'Playfair Display', serif"
              }}>
                      ETB {order.totalAmount}
                  ...
                  <div className="mb-5">
                    <label className="text-xs font-semibold uppercase tracking-wider block mb-3" style={{
                color: '#6B4F3A'
                ...
                }].map(m => <button key={m.method} onClick={() => setPayMethod(m.method)} className="flex flex-col items-center gap-1 py-3 rounded-xl text-xs font-medium transition-all" style={payMethod === m.method ? {
                  background: '#C8862A',
                  color: 'white'
                } : {
                  background: '#F0E8DE',
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => setShowPayModal(false)} className="flex-1 py-3 rounded-xl text-sm" style={{
                background: '#F0E8DE',
                color: '#6B4F3A'
              }}>Cancel</button>
                    <button onClick={processPayment} className="flex-1 py-3 rounded-xl text-sm font-semibold flex items-center justify-center gap-2" style={{
                background: 'linear-gradient(135deg, #059669, #065F46)',
                color: 'white'
              }}>
      ...
      {/* Receipt Modal */}
      {receiptOrder && receiptOrderData && <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{
      background: 'rgba(0,0,0,0.5)'
    }}>
        ...
        background: 'white'
          ...
          background: 'linear-gradient(135deg, #1A1008, #2C1810)'
            ...
            background: 'linear-gradient(135deg, #C8862A, #8B3A0F)'
              ...
              <h3 className="text-white font-black text-lg" style={{
            fontFamily: "'Playfair Display', serif"
          }}>HOLY RESTAURANT</h3>
              <p className="text-xs" style={{
            color: '#8B6E52'
              ...
              <p className="text-xs mt-1" style={{
            color: '#8B6E52'
          }}>Tel: +251 25 111 2345</p>
            </div>
            <div className="p-6">
              <div className="text-center mb-4 pb-4 border-b" style={{
            borderColor: '#F0E8DE',
            borderStyle: 'dashed'
          }}>
                <p className="text-xs" style={{
              color: '#8B6E52'
            }}>Order #{receiptOrderData.orderId}</p>
                <p className="font-bold" style={{
              color: '#2C1810'
            }}>{receiptOrderData.customerName}</p>
                <p className="text-xs" style={{
              color: '#8B6E52'
            }}>{new Date(receiptOrderData.orderDate).toLocaleString()}</p>
              </div>
              <div className="space-y-2 mb-4">
                {receiptOrderData.items.map(item => <div key={item.orderItemId} className="flex justify-between text-sm">
                    <span style={{
                color: '#6B4F3A'
              ...
              <div className="pt-4 border-t mb-4" style={{
            borderColor: '#F0E8DE',
            borderStyle: 'dashed'
          }}>
                ...
                color: '#2C1810'
                  ...
                  <span style={{
                color: '#C8862A'
              }}>ETB {receiptOrderData.totalAmount}</span>
                </div>
                {receiptPaymentData && <p className="text-xs text-center mt-2 capitalize" style={{
              color: '#8B6E52'
            }}>
            ...
            color: '#8B6E52'
              ...
              <button onClick={() => setReceiptOrder(null)} className="w-full py-3 rounded-xl text-sm font-medium" style={{
            background: '#F0E8DE',
            color: '#6B4F3A'
          }}>Close</button>
            </div>
